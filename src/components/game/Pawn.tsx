...
export const Pawn = ({
  ...
  return (
    <div
      className={`relative w-3/4 h-3/4 transition-all duration-300 ease-in-out flex items-center justify-center ${isSelectable ? 'cursor-pointer' : ''
    ...
    >
      <div className={`relative w-full h-full ${isSelectable ? 'animate-bounce' : 'animate-bounce-soft'}`}>
        ...
        <SvgIcon />
        {characterImage && (
          <div className={`absolute flex items-center justify-center w-full h-full ${pawn?.position === -1 ? 'scale-150' : ''} top-0 left-0`}>
            <Image
              src={characterImage}
              alt="character icon"
              width={40}
