...
import type { Operator, PawnState, Player, PlayerColor } from '@/lib/types';
import { produce } from 'immer';
import { ArrowLeft, Dices, HelpCircle, Minus, Plus } from 'lucide-react';
...
export const GameClient = ({ humanColors }: { humanColors: PlayerColor[] }) => {
    ...
    const nextTurn = useCallback((updatedPlayers: Player[] = players) => {
        ...
        if (newWinner) {
            ...
            playSound('win'); // Play victory sound
            handleWin(newWinner);
            return;
        }
    ...
    const handlePawnClick = (pawn: PawnState) => {
        ...
        if (typeof window !== 'undefined' && typeof window.navigator.vibrate === 'function') {
            window.navigator.vibrate(80); // Haptic feedback on pawn click
        }
                ...
                <Card className="p-4 rounded-4xl shadow-lg w-full max-w-md flex flex-col items-center gap-4 bg-white/10 backdrop-blur-sm border border-white/20">
                    <div
                        className={`w-full p-2 rounded-lg flex items-center gap-3 
                          ${winner?.id === 'red' || (!winner && currentPlayer.id === 'red') ? 'bg-red-400' : ''}
                          ${winner?.id === 'green' || (!winner && currentPlayer.id === 'green') ? 'bg-green-400' : ''}
                          ${winner?.id === 'blue' || (!winner && currentPlayer.id === 'blue') ? 'bg-blue-400' : ''}
                          ${winner?.id === 'yellow' || (!winner && currentPlayer.id === 'yellow') ? 'bg-yellow-400' : ''}
                        `}
                    >
                        <Image
