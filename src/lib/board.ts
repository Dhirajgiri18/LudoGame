import type { PawnState, Player, PlayerColor } from './types';
...
const PAWN_CONTAINER_SIZE_PERCENT = `${TILE_SIZE * 1.2}%`; // Board tile pawn size
const PAWN_HOME_SIZE_PERCENT = `${TILE_SIZE * 1.7}%`; // Home base pawn size (bigger)
 ...
 * @param isHome Whether this is a home base pawn (bigger size)
 * @returns CSS properties for positioning.
 */
const pos = (row: number, col: number, isHome = false) => ({
  top: `${(row + 0.5) * TILE_SIZE}%`,
  left: `${(col + 0.5) * TILE_SIZE}%`,
  width: isHome ? PAWN_HOME_SIZE_PERCENT : PAWN_CONTAINER_SIZE_PERCENT,
  height: isHome ? PAWN_HOME_SIZE_PERCENT : PAWN_CONTAINER_SIZE_PERCENT,
 ...
 */
const BASE_POSITIONS: Record<PlayerColor, Array<React.CSSProperties>> = {
    red:    [pos(2, 2, true), pos(2, 5, true), pos(5, 2, true), pos(5, 5, true)],
    green:  [pos(2, 11, true), pos(2, 14, true), pos(5, 11, true), pos(5, 14, true)],
    blue:   [pos(11, 2, true), pos(11, 5, true), pos(14, 2, true), pos(14, 5, true)],
    yellow: [pos(11, 11, true), pos(11, 14, true), pos(14, 11, true), pos(14, 14, true)],
};
