...
import { CheckCircle, Circle, Gamepad2, Palette, Star, Users } from 'lucide-react';
...
const FloatingIcon = ({ icon: Icon, className, duration = 10, delay = 0 }: { icon: React.ElementType, className: string, duration?: number, delay?: number }) => {
  return (
    <div
      ...
      style={{ '--animate-duration': `${duration}s`, '--animate-delay': `${delay}s` } as React.CSSProperties}
      data-animate-delay
    >
          ...
          {colorOptions.map(({ id, hex, name, image, hint }) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-xl font-bold text-white z-10 text-shadow-strong">{name}</span>
