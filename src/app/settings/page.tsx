...
import { ArrowLeft, Music, Paintbrush, RefreshCw, Settings, Star, User, Vibrate } from 'lucide-react';
    ...
    <div className="relative min-h-screen w-full overflow-hidden bg-gradient-to-br from-indigo-950 via-blue-900 to-indigo-950 font-headline flex flex-col items-center justify-center p-4">
      ...
      <Card className="w-full max-w-md shadow-lg rounded-4xl bg-white/90 dark:bg-slate-800/80 backdrop-blur-sm border-white/20 z-10">
        ...
        <CardContent className="space-y-4 sm:space-y-6 p-4 sm:p-6">
          ...
          <div className="flex flex-col gap-2 p-3 sm:p-4 rounded-2xl bg-primary/20">
            <Label htmlFor="volume-slider" className="text-lg sm:text-xl flex items-center gap-2 text-card-foreground">
              Volume
            </Label>
            <Slider id="volume-slider" min={0} max={1} step={0.01} value={[volume]} onValueChange={([v]) => setVolume(v)} className="w-full" />
          </div>
